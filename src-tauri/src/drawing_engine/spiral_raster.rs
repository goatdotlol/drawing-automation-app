...
impl DrawingEngine for SpiralRasterEngine {
    fn process_image(&self, image_data: Vec<u8>, config: &DrawingConfig) -> Vec<DrawingPoint> {
        ...
        let max_radius = (width.max(height) as f32 * 0.71) as u32;
        ...
        while radius < max_radius as f32 {
            ...
            if x < width && y < height {
                ...
                let canvas_x = config.canvas_bounds.x
                    + (x as i32 * config.canvas_bounds.width as i32 / width as i32);
                let canvas_y = config.canvas_bounds.y
                    + (y as i32 * config.canvas_bounds.height as i32 / height as i32);
